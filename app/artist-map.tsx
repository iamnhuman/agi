...
export default function ArtistMap({artists,sections}:{artists:Artist[];sections:Section[]}){
 ...
 // Greedily place longer labels first, choosing the nearest free spot around
 // each point. This keeps the radial map intact while avoiding text collisions.
 const labelBoxes:{left:number;right:number;top:number;bottom:number}[]=[],labelLayout=new Map<string,{x:number;y:number;anchor:'start'|'end';hidden:boolean}>();
 const labelItems=[...nodes].sort((a,b)=>b.artist.name.length-a.artist.name.length);
 for(const node of labelItems){
  const labelWidth=Math.max(24,Math.min(220,Array.from(node.artist.name).length*6.5)),angle=Math.atan2(node.y-node.cy,node.x-node.cx);
  const directions=[angle,angle+Math.PI/2,angle-Math.PI/2,angle+Math.PI,angle+Math.PI/4,angle-Math.PI/4,angle+3*Math.PI/4,angle-3*Math.PI/4];
  let placed:{x:number;y:number;anchor:'start'|'end';box:{left:number;right:number;top:number;bottom:number}}|undefined;
  for(let distance=17;distance<=185&&!placed;distance+=14)for(const direction of directions){
   const dx=Math.cos(direction)*distance,dy=Math.sin(direction)*distance,anchor: 'start'|'end'=dx<0?'end':'start';
   const x=node.x+dx+(dx<0?-9:9),y=node.y+dy+4,left=anchor==='start'?x:x-labelWidth,right=anchor==='start'?x+labelWidth:x;
   const box={left:left-3,right:right+3,top:y-12,bottom:y+4};
   if(box.left<node.cx-columnWidth/2+8||box.right>node.cx+columnWidth/2-8||box.top<node.cy-rowHeight/2+8||box.bottom>node.cy+rowHeight/2-8)continue;
   if(box.left<node.cx+40&&box.right>node.cx-40&&box.top<node.cy+40&&box.bottom>node.cy-40)continue;
   if(labelBoxes.some(other=>box.left<other.right&&box.right>other.left&&box.top<other.bottom&&box.bottom>other.top))continue;
   if(nodes.some(other=>other.artist.id!==node.artist.id&&other.x>box.left-10&&other.x<box.right+10&&other.y>box.top-10&&other.y<box.bottom+10))continue;
   placed={x,y,anchor,box};break;
  }
  if(placed){labelBoxes.push(placed.box);labelLayout.set(node.artist.id,{x:placed.x,y:placed.y,anchor:placed.anchor,hidden:false});}
  else labelLayout.set(node.artist.id,{x:node.x+12,y:node.y+4,anchor:'start',hidden:true});
 }
 const labeledNodes=nodes.map(node=>{const label=labelLayout.get(node.artist.id)!;return {...node,labelX:label.x,labelY:label.y,labelAnchor:label.anchor,labelHidden:label.hidden};});
 ...
 {labeledNodes.map(({artist,x,y,labelX,labelY,labelAnchor,labelHidden},index)=><a data-node="true" data-label-hidden={labelHidden||undefined} className="map-node" key={artist.id} href={artist.url} target="_blank" rel="noopener noreferrer" aria-label={`Открыть источник: ${artist.name}`}><circle cx={x} cy={y} r="16" fill="transparent"/><circle cx={x} cy={y} r={index%7===0?10:7} fill="none" stroke={ratingColor(artist.rating)} strokeWidth="1.5"/><circle cx={x} cy={y} r={index%7===0?6:3.5} fill={artist.section==='world'?'#c9e999':'#bca4d5'}/><text x={labelX} y={labelY} textAnchor={labelAnchor} fill={ratingColor(artist.rating)} stroke="#101510" strokeWidth="3" strokeLinejoin="round" paintOrder="stroke" fontSize="11">{artist.name}</text></a>)}
