...
const kindLabels={artist:'Артисты',media:'Медиа',collective:'Объединения'} as const;
...
export default function ArtistMap({artists,sections,onSelect}:{artists:Artist[];sections:Section[];onSelect:(a:Artist)=>void}){
 ...
 return <div className="map-wrap"><div className="map-note"><span><i/> МИР</span><span><i/> РУНЕТ И ДРУГИЕ</span><p>Типы записей разделены по колонкам · перетаскивайте карту</p></div><svg role="group" aria-label="Карта артистов, медиа и объединений по разделам" viewBox={`0 0 ${width} ${height}`} onPointerDown={event=>{if((event.target as Element).closest('[data-node]'))return;moved.current=false;drag.current={x:event.clientX,y:event.clientY,px:pan.x,py:pan.y};event.currentTarget.setPointerCapture(event.pointerId);}} onPointerMove={event=>{if(!drag.current)return;const scale=width/event.currentTarget.getBoundingClientRect().width;moved.current=true;setPan({x:drag.current.px+(event.clientX-drag.current.x)*scale,y:drag.current.py+(event.clientY-drag.current.y)*scale});}} onPointerUp={()=>drag.current=null} onPointerCancel={()=>drag.current=null} style={{touchAction:'none'}}><defs><pattern id="dots" width="25" height="25" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r=".7" fill="#37402e"/></pattern></defs><rect width={width} height={height} fill="url(#dots)"/><g transform={`translate(${centerX+pan.x} ${centerY+pan.y}) scale(${zoom}) translate(${-centerX} ${-centerY})`}>
