...
export default function Videos({admin=false}:{admin?:boolean}){
 ...
 return <section className="video-section">
  <div className="video-heading"><div><div className="eyebrow">ВИДЕОРАЗВЕДКА · ИИ-СЦЕНА</div><h1>Видеохроника<span>.</span></h1><p>Сигналы ИИ-сцены: смотрите по времени публикации или в порядке поступления в архив.</p></div>{admin&&<button className="primary-btn" onClick={()=>edit()}><Plus size={18}/>Добавить видео</button>}</div>
  <div className="video-toolbar">
   <label className="input-search"><Search size={17}/><input aria-label="Поиск видео" placeholder="Поиск сигнала: автор или видео…" value={q} onChange={e=>setQ(e.target.value)}/></label>
   <div className="video-toolbar-radios">
    <fieldset className="video-radio-group"><legend>ПОРЯДОК</legend><div>
     <label><input type="radio" name="video-order" value="source" checked={order==='source'} onChange={()=>setOrder('source')}/><span>Поступление</span></label>
  ...
  <div className="video-summary">{filtered.length} видеосигналов · {dated.length} с датой · {undated.length} без даты</div>{orientation==='horizontal'&&<p className="timeline-hint">Колесо мыши или перетягивание листает ленту · Ctrl/⌘ + колесо меняет масштаб <span aria-hidden="true">← листайте →</span></p>}
  {error&&<p role="alert" className="error">{error} <button onClick={load}>Повторить</button></p>}
  {loading?<p className="empty">Подключаем видеоархив…</p>:<div className={`timeline-layout timeline-layout--${orientation}${order==='source'?' timeline-layout--source':''}`}>
   ...
   <div ref={timelineViewport} className="timeline-content" aria-label={orientation==='horizontal'?'Горизонтальная видеолента':undefined} tabIndex={orientation==='horizontal'?0:undefined} onPointerDown={orientation==='horizontal'?startDrag:undefined} onPointerMove={orientation==='horizontal'?moveDrag:undefined} onPointerUp={orientation==='horizontal'?stopDrag:undefined} onPointerCancel={orientation==='horizontal'?stopDrag:undefined} onClickCapture={orientation==='horizontal'?event=>{if(suppressDragClick.current){event.preventDefault();event.stopPropagation();suppressDragClick.current=false;}}:undefined} onDragStart={orientation==='horizontal'?event=>event.preventDefault():undefined} onKeyDown={orientation==='horizontal'?event=>{if(event.key==='+'||event.key==='='){event.preventDefault();changeSceneZoom(.1);}else if(event.key==='-'){event.preventDefault();changeSceneZoom(-.1);}}:undefined}>
    {order==='source'?<section className="timeline-year timeline-year--source"><h2>Порядок поступления<sup>{originalOrder.length}</sup></h2><p className="undated-note">Сигналы выстроены так, как поступили в архив, даже если дата неизвестна.</p><div className="timeline-items">{originalOrder.map(card)}</div></section>:<>
