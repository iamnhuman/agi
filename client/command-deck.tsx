 ...
 return <header className="command-deck" aria-label="Главное меню архива">
  <div className="command-monitor">
   <div className="command-screen">
    ...
    <div className="command-screen-content">
     ...
     <AiConquerLogo/>
     <strong>{videos?'ВИДЕОРАЗВЕДКА':'ОПЕРАЦИЯ: ВООБРАЖЕНИЕ'}</strong>
     <span className="command-screen-subtitle">{videos?'ХРОНОЛОГИЯ СИГНАЛОВ ИИ-СЦЕНЫ':'ИИШНИКИ · ХУДОЖНИКИ · МЕДИА'}</span>
    </div>
  ...
  <aside className="command-console">
   ...
   <nav className="command-menu" aria-label="Разделы сайта">
    ...
    <a className={videos?'is-active':''} aria-label="Видеоархив" aria-current={videos?'page':undefined} href="#videos"><span>02</span> ВИДЕОАРХИВ <ArrowUpRight size={15}/></a>
   ...
   <div className="command-console-readout">
    ...
    <div className="command-console-readout-main"><strong>{total??'LIVE'}</strong><span>{videos?'ВИДЕО\nСИГНАЛОВ':'ДОСЬЕ\nВ БАЗЕ'}</span></div>
    <div className="command-console-readout-meter" aria-hidden="true"/>
    <small>{videos?'ХРОНОЛОГИЯ ИИ-СЦЕНЫ':'СОБРАНО ЧЕЛОВЕКОМ / СОЗДАНО С ИИ'}</small>
   </div>
  </aside>
 </header>;
}
