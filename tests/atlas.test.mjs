...
test('video dates, comments, persistence and alternate sources',async t=>{
  ...
  const telegram=videoMetadata('<time datetime="2025-03-04T12:00:00Z"></time><div class="tgme_widget_message_text js-message_text">Новый клип<br>Скоро премьера</div>',post);
  assert.equal(telegram.title,'Новый клип\nСкоро премьера');
  assert.equal(telegram.publishedAt,'2025-03-04');
  const vk=videoLink('https://vk.com/video-12345_67890');
  assert.equal(vk.fetchUrl,'https://vk.com/video-12345_67890');
  assert.equal(videoMetadata('<meta property="og:title" content="VK clip"><meta property="article:published_time" content="2025-04-05T12:00:00Z">',vk).publishedAt,'2025-04-05');
  assert.equal(videoMetadata('<meta property="og:type" content="video.other">',vk).isVideo,true);
  const profile=videoLink('https://vk.com/necofuturism');
  assert.equal(profile.reference,true);
  assert.equal(profile.fetchUrl,'https://vk.com/necofuturism');
  const {importVideo}=await import('../server/video-import.mjs');
  let requested='';
  const imported=await importVideo('https://vk.com/necofuturism',async url=>{requested=url;return new Response('<meta property="og:image" content="/images/cover.jpg"><meta property="og:title" content="Neco Futurism">',{headers:{'content-type':'text/html'}});});
  assert.equal(requested,'https://vk.com/necofuturism');
  assert.equal(imported.image,'https://vk.com/images/cover.jpg');
  assert.equal(imported.isVideo,false);
  assert.equal(videoMetadata('<meta name="twitter:image" content="//cdn.example/image.jpg">',profile).image,'https://cdn.example/image.jpg');
  const {file}=await fixture(t);const store=createCatalogStore(file);
